...
async function recordSuccessfulCleaning(item: BatchItem) {
  ...
  if (!response.ok) { if (response.status === 401) throw new Error('Sign in to unlock your download. Your cleaned image is still on this device.'); throw new Error(data.error || 'NoMeta could not confirm your cleaning allowance. Please try again.'); }
}
...
export function BatchExperience({ files, onReset }: { files: UploadItem[]; onReset?: () => void }) {
  ...
  const addFiles = (incoming: FileList | File[]) => {
    ...
    for (const file of Array.from(incoming)) {
      ...
      const previewUrl = URL.createObjectURL(file); urlsRef.current.push(previewUrl);
      next.push({ id: makeId(), file, previewUrl, status: 'ready', stage: 'queued' }); existingKeys.add(key);
    }
    setItems(next); setAddError(rejected); if (inputRef.current) inputRef.current.value = '';
  };
  const removeItem = (id: string) => { if (busy) return; const item = items.find(i => i.id === id); if (item) { URL.revokeObjectURL(item.previewUrl); if (item.downloadUrl) URL.revokeObjectURL(item.downloadUrl); } setItems(prev => prev.filter(i => i.id !== id)); };
  const confirmEntitlement = async (item: BatchItem) => { try { await recordSuccessfulCleaning(item); update(item.id, { stage: 'verified', error: undefined }); return true; } catch (e) { update(item.id, { stage: 'awaiting_entitlement', error: e instanceof Error ? e.message : 'Unable to confirm your cleaning allowance.' }); return false; } };
  const runWorkflow = async () => {
    if (busy || (!queued && !scannedReady)) return; setBusy(true); abortRef.current = new AbortController();
    try {
      const targets = items.filter(i => i.stage === 'queued' || i.stage === 'scanned');
      for (const original of targets) {
        if (abortRef.current.signal.aborted) break; let item = original;
        if (item.stage === 'queued') { update(item.id, { stage: 'scanning', error: undefined }); try { const result = await scanImage(item.file); item = { ...item, stage: 'scanned', result, error: undefined }; update(item.id, { stage: 'scanned', result, error: undefined }); } catch (e) { update(item.id, { stage: 'error', error: e instanceof Error ? e.message : 'Unable to scan this image.' }); continue; } }
        if (!item.result || abortRef.current.signal.aborted) continue;
        update(item.id, { stage: 'cleaning', error: undefined });
        try { const blob = await cleanImage(item.file, CLEANING_MODE); const check = await verifyCleanedImage(blob); if (!check.verified) throw new Error(`${check.remainingMetadata} supported metadata item(s) remain after cleaning.`); const url = URL.createObjectURL(blob); urlsRef.current.push(url); const prepared: BatchItem = { ...item, stage: 'awaiting_entitlement', cleanedBlob: blob, downloadUrl: url, cleanedMode: CLEANING_MODE, error: undefined }; update(item.id, prepared); await confirmEntitlement(prepared); } catch (e) { update(item.id, { stage: 'error', error: e instanceof Error ? e.message : 'Cleaning failed.' }); }
      }
  ...
  const retryFailed = () => { if (busy) return; setItems(prev => prev.map(item => item.stage === 'error' ? { ...item, stage: 'queued', error: undefined } : item)); };
  ...
  const downloadAll = async () => { if (!cleaned.length || zipBusy) return; if (cleaned.length === 1) { downloadOne(cleaned[0]); return; } setZipBusy(true); try { const zip = await createLocalZip(cleaned.map(i => ({ name: outputName(i.file.name), blob: i.cleanedBlob! }))); const url = URL.createObjectURL(zip); triggerDownload(url, `nometa-cleaned-${new Date().toISOString().slice(0, 10)}.zip`); window.setTimeout(() => URL.revokeObjectURL(url), 1500); } catch {} finally { setZipBusy(false); } };
  const statusText = useMemo(() => { if (!items.length) return 'No photos selected'; if (completed === items.length) return `${completed} ${completed === 1 ? 'photo is' : 'photos are'} ready to download`; if (cleaning > 0) return `Cleaning ${cleaning} photo${cleaning === 1 ? '' : 's'}...`; if (awaiting > 0) return `Confirming ${awaiting} cleaned photo${awaiting === 1 ? '' : 's'}...`; if (scanning > 0) return `Scanning ${scanning} photo${scanning === 1 ? '' : 's'}...`; if (queued > 0 || scannedReady > 0) return `${queued + scannedReady} photo${queued + scannedReady === 1 ? '' : 's'} ready when you are`; if (failed > 0) return `${failed} photo${failed === 1 ? '' : 's'} need attention`; return 'Your private photo workspace'; }, [items.length, completed, cleaning, awaiting, scanning, queued, scannedReady, failed]);
  ...
  return <section className="nm-batch" aria-live="polite">
    ...
    <div className="nm-batch__header"><div><span className="nm-eyebrow">Private photo workspace</span><h2>{statusText}</h2><p>{items.length} photo{items.length === 1 ? '' : 's'} · Nothing is uploaded during image processing.</p></div><div className="nm-batch__actions">
      {!busy ? <Button variant="secondary" onClick={() => inputRef.current?.click()} icon="upload">Add photos</Button> : <Button variant="secondary" onClick={cancel}>Stop</Button>}
      ...
      {!busy && completed > 0 ? <Button variant="primary" onClick={downloadAll} disabled={zipBusy} icon="download">{zipBusy ? 'Preparing download...' : completed > 1 ? 'Download all' : 'Download photo'}</Button> : null}
    </div></div>
    <div className="nm-batch__progress" aria-label={`${progress}% complete`}><div className="nm-batch__progress-top"><span>{progress === 100 ? 'Complete' : busy ? 'Working securely on your device' : 'Workflow progress'}</span><strong>{progress}%</strong></div><div className="nm-batch__progress-track"><span style={{ width: `${progress}%` }} /></div><small className="nm-batch__progress-help">{scanning > 0 ? 'Reading the photo and checking supported metadata' : cleaning > 0 ? 'Creating and verifying your clean copy' : completed > 0 ? 'Your clean copy has been verified' : progress === 0 ? 'Nothing starts until you press Scan & clean' : 'Almost there'}</small></div>
    ...
    <div className="nm-batch__list">{items.map((item, index) => <article className="nm-batch-item" key={item.id}><img src={item.previewUrl} alt="" /><div className="nm-batch-item__body"><strong title={item.file.name}>{item.file.name}</strong><span>{formatFileSize(item.file.size)} · {item.result ? `${item.result.entries.length} metadata item${item.result.entries.length === 1 ? '' : 's'} found` : 'Waiting to scan'}</span>{item.error ? <small className="nm-batch-item__error">{item.error}</small> : null}</div><div className={`nm-batch-status nm-batch-status--${item.stage}`}><i aria-hidden="true" />{label(item.stage, index + 1, items.length)}</div>{!busy && ['queued', 'error'].includes(item.stage) ? <button className="nm-file-card__remove nm-batch-item__remove" type="button" aria-label={`Remove ${item.file.name}`} onClick={() => removeItem(item.id)}>×</button> : null}{item.stage === 'awaiting_entitlement' ? <div className="nm-batch-item__actions"><button type="button" className="nm-button nm-button--secondary nm-batch-download" onClick={() => retryEntitlement(item)} disabled={busy}>Retry confirmation</button></div> : null}{item.stage === 'verified' && item.downloadUrl ? <div className="nm-batch-item__actions"><button type="button" className="nm-button nm-button--secondary nm-batch-download" onClick={() => downloadOne(item)}>Download</button></div> : null}</article>)}</div>
